...
Class Action {
	...
	function delete_user(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM user where userID = ".$id);
		if($delete)
			return 1;
	}
	function signup(){
	...
	function save_plan(){
		...
		$data = " plan = '$plan' ";
			...
			if(empty($id)){
				...
				$save = $this->db->query("UPDATE plans set $data where planID = $plan");
			}
		if($save)
			return 1;
	}
	function delete_plan(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM plans where planID = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_trainer(){
		...
		$data .= ", contact = '$contact' ";
		$data .= ", email = '$email' ";
		$data .= ", achievement = '$achievment' ";
		$data .= ", description = '$description' ";
		if($_FILES['img']['tmp_name'] != ''){
						$file = $_FILES['img']['name'];
						$move = move_uploaded_file($_FILES['img']['tmp_name'],'assets/trainers/'. $file);
					$data .= ", image = '$file' ";

		}
		$data .= ", speciality = '$speciality' ";
		$data .= ", class = '".$time_from.' To '.$time_to."' ";
			if(empty($id)){
