					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$plan = $conn->query("SELECT * FROM plans order by plan asc");
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_plan" type="button" data-id="<?php echo $row['planID'] ?>" data-plan="<?php echo $row['plan'] ?>"   >Edit</button>
...
<script>
	...
	$('.edit_plan').click(function(){
		...
		var cat = $('#manage-plan')
		cat.get(0).reset()
		cat.find("[name='planID']").val($(this).attr('data-id'))
